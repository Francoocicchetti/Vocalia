...
with tempfile.TemporaryDirectory(prefix='VocaliaEngineTest-') as folder:
    ...
    for index, (task, source, expected) in enumerate(cases):
        ...
        job = dict(task=task, model='tiny', models=str(models), source=str(source), language='en', terms=[])
        ...
        if not expected:
            ...
            print('PASS: corrupted audio rejected without closing the parent', flush=True)
            continue
