...
ditto -c -k --norsrc --keepParent "$APP_DIR" "$OUTPUT_DIR/Vocalia-1.0.3-macOS-AppleSilicon.zip"
cp "$SOURCE_DIR/START-HERE.txt" "$BUILD_DIR/START-HERE.txt"
(cd "$BUILD_DIR" && /usr/bin/zip -q "$OUTPUT_DIR/Vocalia-1.0.3-macOS-AppleSilicon.zip" START-HERE.txt)
echo "Build ready: $OUTPUT_DIR/Vocalia-1.0.3-macOS-AppleSilicon.zip"
